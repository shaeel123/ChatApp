...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (currState === "Sign Up") {
        ...
        // ✅ Sign out then send OTP (not magic link)
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const { data, error } = await supabase.auth.verifyOtp({
        email,
        token: otpCode,
        type: currState === 'Sign Up' ? 'signup' : 'email', // ✅ was 'magiclink'
      })

      if (error) throw error

      // ✅ Create profile here — user is now authenticated
  ...
  const handleResendOtp = async () => {
    try {
      const type = currState === 'Sign Up' ? 'signup' : 'email' // ✅ was hardcoded 'signup'
